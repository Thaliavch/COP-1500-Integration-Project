...
# A while loop is one of the two looping structures available in Python, and
# it is usually used when the number of times the function has to run is
# unknown.
# Parts of a while loop: Initialization(variable), Condition,
# Increment(update variable)
# A sentinel-controlled while loop is a loop that repeats the loop body until
# the user enters a pre-
...
# between two operands. Expressions whose result can only be true or false are
# known as Boolean
# expressions. In this case the next line of coding is checking if the value
# stored in
# the variable "age" is less than 18.
if age < 18:
    print("Sorry, to use our services you need to be eighteen years or older.")
else:
    print("Thank you for your interest in using our services.")
    # Determine how much time the RBT will spend doing paper work outside of
    # session and the total of hours working per week.
    # Determine if the RBT currently has a part-time or full-time job
    outwork = input("Are you interested in knowing the average time you may "
                    "spend working outside of session?")
    while True:
        # An if else statement is a conditional statement that runs a different
        # set of statements depending on whether an expression is true or false
        # In our next line of code the condition is "outwork == 'Yes'"
        if outwork == "Yes":
            break
        # elif is the Python keyword that represents else if and allows you to
        # test for one of several options.
    ...
    if outwork == y:
        while True:
            try:
                sessions = int(
                    input("How many sessions do you have per week?"))
                break
            except:
                print("Please enter a valid number")

        # numeric operators: *(multiplication), //(floor division), %(modulus),
        # +(addition).
        # numeric operators: **(exponent), -(subtraction), /(division).
        extra = 15 * sessions
        extrah = int(extra // 60)
        extram = int(extra % 60)
        # Relational operators or Conditional operator "!=" means "it is not
        # equal to."
        if extrah != 1:
            # string operator ","(concatenates a string with an int/float and
            # with other objects like end= and sep=, ex: ("hi", 2) == hi2)
            print("Expect to spend ", extrah, "hours, and ", end=" ")
        elif extrah == 1:
            print("Expect to spend", extrah, "hour, and", sep=" ", end=" ")
        if extram != 1:
            print(extram, " minutes each week doing outside paperwork.",
                  sep="")
        elif extram == 1:
            print(extram, " minute each week doing outside paperwork.", sep="")
        # Asking the total amount of hours working in the client's case
        week = input("Do you want to know about how many hours per week you'll"
                     " be working when including time outside session?")
        while True:
            if week == "Yes":
                break
            elif week == "No":
        ...
        if week == y:
            ...
            total = extra + time * sessions
            hours = int(total // 60)
            minutes = int(total % 60)
            print("You are expected to \"work\" a total of ", hours,
    ...
    # Reinforcement:
    # ...Pairing...
    print("Pairing Session Assessment \nPlease, answer the next questions with "
          "\"Yes\" or \"No\"")
    ...
    # "and", "or", and "not." In the next kind of code the program if saying
    # that it is True
    # if obj1, and obj2, and obj3, and so on are equal to the value stored in
    # the variable "y."
    if (obj1 == y and obj2 == y and obj3 == y and obj4 == y and obj5 == y and
            obj6 == y):
        print("Perfect! This means that your client likes you, and now you can"
              " spend more time teaching instead of battling bad behavior.",
              end=" ")
    ...
    # Relational operators or Conditional operator "==" means "is equal to."
            ...
            obj1 == y or obj2 == y or obj3 == y or obj4 == y or obj5 == y
            or obj6 == y):
        print(
            "There is still work to do. \n Remember that is very important to"
            " gain the trust of the costumers", end="")
    ...
    print("Ask the caregiver the next questions, and write down their answers"
          " to later show them to your BCBA.")
    dailyact = input("What did they do today for fun?")
    alonebeh = input("If left alone in a room what do they do?")
    ...
    obji_list = []
    # The Python predefined function - range() - is used to define a series of
    ...
    for x in range(1, 5 + 1):
        ...
        obji = input("Item number " + str(x) + " :")
        obji_list.append(obji)
    ...
    print("After knowing the results from the indirect preference assessment,"
          " pick five more objects to conduct the direct"
          " preference assessment.")
    objd_list = []
    count = 1
    while count < 6:
        objd = input("Item number " + str(count) + " :")
        objd_list.append(objd)
        ...
        # short-cut operator (+=) is usually used for incrementing a variable.
        # One example is the
        # next line of code.
        count += 1
    objd_list.extend(obji_list)
    print("These are your direct preference assessment materials: ")
    for count1 in objd_list:
        print("." + count1, end="  ")
    print()
    ...
    for rounds in range(num_rounds):
        for objects in range(num_objects):
            objects_count = random.choice(objd_list)
    ...
    print("###################################################################"
          "#############################################\n")
# This code is to use some numeric operators that I did not have the chance to
# use in this program because it is not needed(Sprint #1)

# In January a factory starts building a total of "x" chairs per week, but the
# production of chairs starts doubling in size each month,
...
# business in January (note that in January all chairs produced were sold)?
print("Bonus code #0.\n")
...
def main():  # function definition of a void function.
    x = int(input("How many chairs did the factory produce in January?"))
    y = int(input("How many chairs did the factory donate on December of that "
                  "same year?"))
    w = int(input("How much does a chair costs?"))
    ...
    print("After donating " + str(y) + " chairs on December, the factory will"
        " have", format(q2_chair(x, y), "7d"), " chairs left to sell.", sep="")
